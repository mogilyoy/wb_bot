...
def starts(message):
    bot.send_message(
        message.chat.id,
        "Тут вы можете оформить поставочку: ",
...
def callbacks(callback):
    ...
    if callback.data == "supplies":
        bot.edit_message_text(
            "Меню поставочек хуй знает зачем оно нужно",
            callback.from_user.id,
            callback.message.message_id,
            reply_markup=markups.supplies_menu(),
        )
    ...
    if callback.data == "set_order_st1":
        ...
        if status == True:
            bot.edit_message_text(
                "Товары успешно добавлены на сборку!",
                callback.from_user.id,
                callback.message.message_id,
                reply_markup=markups.menu(),
            )
    ...
    if callback.data == "stickers_barcode":
        try:
            ...
            if orders:
                ...
                bot.edit_message_text(
                    "Наклейте на ебаные посылочки",
                    callback.from_user.id,
                    callback.message.message_id,
                    reply_markup=markups.print_sticker_menu(),
                )
                bot.send_document(
                    callback.from_user.id, document=open("images/img_orders.jpg", "rb")
                )
                for order in order_list:
                    print(wb.set_order_status(order, 2))
            else:
                bot.edit_message_text(
                    "В поставке нет заказов",
                    callback.from_user.id,
                    callback.message.message_id,
                    reply_markup=markups.menu(),
                )
    ...
    if callback.data == "supply_barcode":
        try:
            ...
            bot.edit_message_text(
                "Вот ваш ебаный баркод поставки",
                callback.from_user.id,
                callback.message.message_id,
                reply_markup=markups.print_supply_menu(),
            )
            bot.send_document(
                callback.from_user.id,
                document=open("images/img_with_watermark.jpg", "rb"),
            )
